...
import { TNL, TNLBalanced } from '../dist/index';
import { TNLTypes } from '../dist/index';
export function sleep(milliseconds: number) {
  return new Promise(resolve => setTimeout(resolve, milliseconds));
}

const TNL_API_KEY = '39bf4ff1-bfb9-4c4a-bde3-af10b0d3fbd3';
...
describe('A REALLY QUICK TNL TEST', () => {
  ...
  it('should set the first setting ', async () => {
    ...
    console.log(
      (r2.response as TNLTypes.WebhookResponses.Settings).buttons[2],
    );
...
const tnllb = new TNLBalanced(TNL_API_KEY);
describe('A REALLY QUICK TNL BALANCED TEST', () => {
  it('should create an imagine command and have a response ', async () => {
    const imagineRes = await tnllb.imagine('A cat playing piano');
    console.log(imagineRes);

    await sleep(60000);

    const messageProgresss = await tnllb.getMessageAndProgress(
      imagineRes.messageId,
      imagineRes.loadBalanceId
    );

    console.log(`Progress: ${messageProgresss.progress}`);

    const buttonRes = await tnllb.button(
      messageProgresss.response.buttons[0] as TNLTypes.ButtonTypes,
      '3VVNxa96OIBhpiLVxQFs',
    );
    console.log(buttonRes);
